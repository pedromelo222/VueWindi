...
const getConfigPath = () => {
  ...
  if (!configFilePath) throw new Error("windi.config file not found");

  return configFilePath;
};

const resolveConfig = (configPath: string) => {
  return require(configPath).default.vueWindi;
};

const getTheme = () => {
  const themePath = getConfigPath();
  const getTheme = resolveConfig(themePath);
  ...
  if (getTheme === undefined || getTheme.themes === undefined) {
    ...
    if (!isObjKey(definedTheme as string, themes)) definedTheme = DEFAULT_THEME;
